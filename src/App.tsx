    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen gradient-bg flex items-center justify-center p-6 text-slate-100">
          <div className="max-w-md rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-xl">
            <h1 className="text-xl font-semibold">Dashboard could not load</h1>
            <p className="mt-2 text-sm text-slate-300">Please refresh the page. Your imported data remains safely stored in this browser.</p>
          </div>
        </div>
      );
    }
        ...
        <Suspense fallback={<LoadingPage />}>
          <Routes>
            <Route element={<AppLayout darkMode={settings.darkMode} onToggleDarkMode={toggleDarkMode} />}>
              ...
              <Route path="/settings" element={<SettingsPage settings={settings} updateSettings={updateSettings} updateWeights={updateWeights} resetToDefaults={resetToDefaults} />} />
            </Route>
          </Routes>
        </Suspense>
      </PageErrorBoundary>
...
export default function App() {
  return <AppStateProvider><AppInner /></AppStateProvider>;
}
